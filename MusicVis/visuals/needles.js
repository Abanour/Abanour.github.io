	...
	this.draw = function(){
		...
		for(var i = 0; i < this.plotsDown; i++){
			for(var j = 0; j < this.plotsAcross; j++){
				...
				var x = 70;
				var y = 70;
				var w = this.plotWidth - x;
				var h = this.plotHeight - y;
				
				//draw a rectangle at that location and size
				rect(x + (h * 2 * j), y + (h * 1.25 * i), w, h);
				
				//add on the ticks
				// ??
				this.ticks((w / 2) + (x + (h * 2 * j)), h + y + (h * 1.25 * i), this.frequencyBins[currentBin]);
				
				var energy = fourier.getEnergy(this.frequencyBins[currentBin]);
				
				//add the needle
				// ??
				this.needle(energy, (w/ 2) + x + (h * 2 * j), h + y + (h * 1.25 * i));
